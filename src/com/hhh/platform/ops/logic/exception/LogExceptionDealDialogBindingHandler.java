package com.hhh.platform.ops.logic.exception;
...
public class LogExceptionDealDialogBindingHandler extends DefaultBindingHandler {
	private static Log log = LogFactory.getLog(LogExceptionDealDialogBindingHandler.class);

	private String operationId;
	private LogExceptionEntity entity;
	private HashMap<String, Object> savedValues = new HashMap<String, Object>();

	public LogExceptionDealDialogBindingHandler(String operationId, LogExceptionEntity entity) {
	...
	public boolean perform(String action) {
		if (action.equals(BindingHandler.CONFIRM_ACTION) && this.validate(true)) {
			if (this.operationId.equals(OPSConstants.DEAL_LOG_EXCEPTION)) {
				try {
					updateLog();
					return true;
				} catch (Exception e) {
					String message = "修改异常日志失败";
	...
	private Map updateLog() throws Exception {
		...
		for (int selecedIndex = 0; selecedIndex < logIds.length; selecedIndex++) {
			...
			String sql = "com.hhh.platform.ops.sql.ops_log_exceptionMapper.updateLog";
