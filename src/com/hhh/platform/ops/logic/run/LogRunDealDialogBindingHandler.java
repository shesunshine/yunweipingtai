...
public class LogRunDealDialogBindingHandler extends DefaultBindingHandler {
	private static Log log = LogFactory.getLog(LogRunDealDialogBindingHandler.class);

	private String operationId;
	private LogRunEntity entity;
	private HashMap<String, Object> savedValues = new HashMap<String, Object>();

	public LogRunDealDialogBindingHandler(String operationId, LogRunEntity entity) {
	...
	public boolean perform(String action) {
		if (action.equals(BindingHandler.CONFIRM_ACTION) && this.validate(true)) {
			if (this.operationId.equals(OPSConstants.DEAL_LOG_RUN)) {
				try {
					updateLog();
					return true;
				} catch (Exception e) {
					String message = "修改运行日志失败";
	...
	private Map updateLog() throws Exception {
		...
		for (int selecedIndex = 0; selecedIndex < logIds.length; selecedIndex++) {
			...
			parameters.put("log_id", logIds[selecedIndex]);
			parameters.put(FrameworkConstants.UPDATEVALUES, updateValues);
			String sql = "com.hhh.platform.ops.sql.ops_log_runMapper.updateLog";
